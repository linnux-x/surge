...
| P2，已修复（后续补丁） | 直接只指定服务规则时，旧 Global 已裁剪的规则无法恢复 | 生成器入口使用 `OVERLAP_DEPENDENTS` 自动补充 Global。回归测试先在旧代码复现失败，再验证服务删除规则后 Global 恢复覆盖；空选择和无关规则不扩展 |
...
2026-09-08 后续修复：生成器已自行补齐 Global 增量依赖，单元测试增至 57 项。整批 staging、路径与解析接口调整仍为后续计划。
