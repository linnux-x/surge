...
- When adding a dedicated `Download.list`, place it before `Game.list` if game download CDN hosts should follow the Download policy, then exclude those exact download/CDN entries from `Game.list` to keep Game focused on game services. Keep Mainland China game download/cache endpoints in `China.list` or a direct Download policy according to the active first-match order.
